...
export function AttackLabBody() {
  ...
  const play = (res: AttackLab) => {
    ...
    setPubRevealed(0);
    setPrivRevealed(0);
    // Reveal the stages in order across a fixed window, so both columns finish
    // together. The pacing is presentation — the model is computed the instant
    // the response lands — so it is spread evenly rather than dressed up with
    // per-step delays that would imply a measured duration.
    const schedule = (steps: AttackStep[], set: (n: number) => void) => {
      steps.forEach((_, i) => {
        const delay = ((i + 1) / steps.length) * PLAY_MS;
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground flex items-center gap-1.5">
          ...
          Same {formatUsd(1000, 0)} mUSD · 10x {side} on {symbol}. A front-running bot attacks it on a
      ...
      {!result && !error && (
        <div className="rounded-lg border border-dashed border-border p-8 text-center text-xs text-muted-foreground">
          Solve the attack to see what a sandwich bot would take on a transparent venue — and why
          the same attack cannot be constructed against a sealed order.
        </div>
      )}
...
/**
 * The line between what is computed and what is measured, stated where nobody
 * can miss it.
 *
 * Everything in this dialog is solved against live inputs — this market's vAMM
 * curve, the current Chainlink index, a real SHA-256 preimage search — but none
 * of it touches a chain. The app measures the same attack for real over on
 * /mev, with transaction hashes, so the only honest thing to do is say which
 * one you are looking at and point at the other.
 */
function ModelBanner() {
  return (
    <div className="rounded-md border border-warning/50 bg-warning/10 px-3 py-2 space-y-1">
      <div className="flex items-center gap-1.5">
        <FlaskConical className="size-3.5 text-warning shrink-0" />
        <span className="text-[10px] font-bold uppercase tracking-widest text-warning">
          Model — not an on-chain measurement
        </span>
      </div>
      <p className="text-[11px] leading-snug text-muted-foreground">
        Solved live against this market&apos;s vAMM curve and its Chainlink index price. Real
        inputs and real arithmetic, but no transaction is sent and nothing here happened on a
        chain.{" "}
        <Link href="/mev" className="text-primary underline underline-offset-2">
          MEV Shield runs this attack for real on Sepolia
        </Link>{" "}
        — same claim, with transaction hashes.
      </p>
    </div>
  );
}
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="shrink-0 gap-1.5" title="Attack Lab">
