...
/**
 * Fetch prior candles from Pyth's public TradingView shim (no key).
 *
 * Backfill only — so the chart opens with history instead of a blank pane. The
 * mark and index lines the terminal actually trades against come from the
 * operator's Chainlink reads, never from here. Returns
...
export default function TradingChart() {
  ...
  const { selectedMarketId, setSelectedMarketId, reconcile } = useMarketSelection();
  const { data: markets, isError } = useMarkets();

  // The chart owns the market picker, so it is the one place that always knows
  // the live list. A selection restored from storage — or the built-in default —
  // can name a market the operator no longer quotes; without this the terminal
  // would sit on a market with no prices and no explanation.
  useEffect(() => {
    if (markets) reconcile(markets.map((m) => m.id));
  }, [markets, reconcile]);
  ...
  // Backfill history from Pyth's public shim on market/timeframe change, then
      ...
      <Card className="h-full flex flex-col">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 px-3 sm:px-4 pb-2">
          ...
          <div className="ml-auto flex items-center gap-1.5 text-[10px] text-muted-foreground font-mono">
            ...
            <span className="hidden md:inline">{tfLabel} · vAMM mark vs Chainlink index</span>
          </div>
        </div>
