...
/** Close-flow: settlement proof → Sepolia settlement, live. */
...
function MarginControl({
  ...
  const submit = async (sign: 1 | -1) => {
    ...
    try {
      ...
      toast.success(sign > 0 ? `Added ${formatUsd(amt)} mUSD margin` : `Removed ${formatUsd(amt)} mUSD margin`);
      setOpen(false);
      setAmount("");
      onAdjusted();
    } catch (e: any) {
...
export default function Portfolio() {
  ...
  const { connected } = useDorrWallet();
  // Disconnected, follow a real funded account rather than showing an empty
  // panel — see `useViewedAddress`. `canAct` gates everything that moves value.
      ...
      <CardContent className="p-0 flex-1 min-h-0 overflow-y-auto flex flex-col">
        ...
        {isSpectator && address && (
          <div className="px-4 pt-3">
            <SpectatorNotice address={address} />
          </div>
        )}
            ...
            {rows.map((p) => (
              ...
              >
                ...
                <div className="hidden lg:grid lg:grid-cols-11 gap-2 px-4 py-3 text-xs font-mono items-center hover:bg-muted/30 transition-colors">
                  ...
                  <span className="text-right">{formatSize(p.sizeBase)}</span>
                ...
                <div className="lg:hidden px-4 py-3 space-y-2">
                  ...
                  {p.status === "open" && !canAct && (
                    <div className="pt-1 text-[10px] text-muted-foreground">watching</div>
                  )}
                  {p.status === "open" && canAct && (
