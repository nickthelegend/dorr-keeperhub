...
import { useSettlements, useFeed } from "@/hooks/use-operator";
 ...
 * The second tab is the evidence: every PnL `DorrVault` has actually paid,
 * read from its own `PnlApplied` logs. Only KeeperHub's wallet can cause one,
 * so this is the chain corroborating the operator rather than the operator
 * corroborating itself.
 */
export default function PublicFeed() {
  const [tab, setTab] = useState<"feed" | "settled">("feed");
  const { data: feed, isError: feedError } = useFeed();
  const { data: settlements, isError: settleError } = useSettlements(25);
        ...
        action={
          <div className="flex gap-1 p-0.5 bg-muted rounded-md">
            {(["feed", "settled"] as const).map((t) => (
              ...
              >
                {t === "feed" ? "order flow" : "settled on chain"}
              </button>
            ))}
          </div>
        }
      ...
      <CardContent className="flex-1 min-h-0 overflow-y-auto space-y-1.5">
        {tab === "feed" ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
            ...
            <div className="text-xs text-muted-foreground">
              {settleError ? "Vault unreachable." : "Nothing settled yet — close a position."}
            </div>
          </div>
        ) : (
          settlements.map((sx, i) => (
            <div
              key={`${sx.txHash}-${sx.trader}-${i}`}
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[11px] font-mono">{truncateHash(sx.trader, 6, 4)}</span>
                </div>
                <span
                  className={cn(
                    "text-[11px] font-mono tabular-nums",
                    sx.delta > 0 ? "text-success" : "text-destructive",
                  )}
                >
                  {sx.delta > 0 ? "+" : "−"}
                  {Math.abs(sx.delta).toFixed(4)} mUSD
                </span>
              </div>
              <div className="text-[10px] font-mono text-muted-foreground">
                block {sx.blockNumber.toLocaleString()} · balance now {sx.newBalance.toFixed(2)}
              </div>
              <a
                href={sx.explorerUrl}
                target="_blank"
              ...
              >
                {truncateHash(sx.txHash, 10, 6)} <ExternalLink className="w-2.5 h-2.5" />
              </a>
            </div>
          ))
        )}
