...
export default function TradingPanel() {
  ...
  // Follow the same account the rest of the terminal shows, so the free balance
  // here cannot disagree with the collateral panel three inches below it.
  // Submitting still requires `connected` — see `handleCommit` and the button.
  const { address: viewedAddress, isSpectator } = useViewedAddress();
  const { data: account } = useAccount(viewedAddress);
  ...
  useEffect(() => {
    ...
    if (execJob.data.status === "complete") {
      ...
      toast.success(`${side} position opened`, {
        description: fill ? `${formatSize(fill.sizeBase)} ${market?.base ?? ""} @ ${formatUsd(fill.entryPrice)}` : undefined,
      });
  ...
  const handleSubmit = async () => {
    ...
    if (account && marginNum > account.free) {
      toast.error(`Insufficient free balance (${formatUsd(account.free)} mUSD). Deposit or faucet first.`);
      return;
    }
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-[10px] text-muted-foreground">
            <span>{isSpectator ? "Free balance (watching)" : "Free balance"}</span>
            <span className="font-mono">
              {account ? `${formatUsd(account.free)} mUSD` : "…"}
            </span>
          </div>
        </div>
        ...
        {marginNum > 0 && (
          <div className="space-y-1 text-xs p-3 bg-muted/40 rounded-lg border border-border/60">
            <div className="flex justify-between">
              ...
              <span className="font-mono font-semibold">{formatUsd(marginNum * leverage)} mUSD</span>
            </div>
            <div className="flex justify-between">
        ...
        {commit && (
          ...
          >
            ...
            <div className="flex justify-between text-[10px] text-muted-foreground font-mono">
              <span>size {formatSize(commit.sizeBase)}</span>
        ...
        {(phase === "commit-job" ||
          ...
          <div className="rounded-lg border border-border/60 bg-muted/20 p-3 space-y-3">
            {commitJob.data && <JobProgress job={commitJob.data} title="commit · sealing the order" />}
        ...
        {phase === "done" && fill && (
          ...
          >
            ...
            <div className="flex justify-between font-mono">
              <span>
                {formatSize(fill.sizeBase)} {market?.base}
              </span>
              <span>@ {formatUsd(fill.entryPrice)} mUSD</span>
            </div>
          </motion.div>
        )}
        ...
        {phase === "rested" && (
          ...
          >
            ...
            <div className="flex justify-between font-mono text-muted-foreground">
              ...
              <span>@ {formatUsd(limitNum || null)} mUSD</span>
            </div>
