...
/**
 * EVM wallet connect for Sepolia Sepolia (MetaMask, Rabby, Brave, …).
 *
 * dorr margins in mUSD — an ERC-20 on Sepolia — so the connected account has to be
...
export function WalletConnectButton({ className }: { className?: string }) {
  const {
    ...
    available, wrongNetwork, switchToSepolia,
  } = useDorrWallet();

  const handleConnect = async () => {
    if (!available) {
      toast.error("No EVM wallet found", {
        description: "Install MetaMask or Rabby to trade on Sepolia.",
  ...
  if (!connected) {
    return (
      ...
      >
        {connecting ? (
          <Loader2 className="size-4 animate-spin xs:mr-2" />
        ) : (
          <Wallet className="size-4 xs:mr-2" />
        )}
        {/*
          The label sheds words as the bar gets tighter. At 320px the icon
          carries it alone — with a title and aria-label, so it stays legible to
          a screen reader and on hover even when the text is gone.
        */}
        {connecting ? (
  ...
  if (wrongNetwork) {
    return (
      <Button
        onClick={() => switchToSepolia().catch(() => toast.error("Network switch rejected"))}
