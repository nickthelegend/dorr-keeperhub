...
/** Operator configuration — chain, contracts, and the spectator account. */
export function useConfig() {
  return useQuery({
    queryKey: ["config"],
    queryFn: () => operator.config(),
    staleTime: 5 * 60_000,
    retry: false,
  });
}

export function useSettlements(limit = 25) {
  return useQuery({
    queryKey: ["settlements", limit],
    queryFn: () => operator.settlements(limit),
