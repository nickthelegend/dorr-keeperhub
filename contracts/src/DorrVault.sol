...
/// @title DorrVault — non-custodial collateral margin vault
/// @notice Collateral for dorr perpetual futures, held in an ERC-20 (mUSD on Sepolia).
///
/// THE INVARIANT THAT MATTERS:
///   collateral can leave this contract in exactly ONE way — the depositor calling
...
contract DorrVault is ReentrancyGuard, Ownable {
    ...
    /// @notice The margin token (mUSD on Sepolia, 18 decimals).
    ...
    /// @notice Sum of all internal balances. Must always be <= collateral.balanceOf(this).
    ...
    /// @notice Deposit collateral as margin collateral. Caller must have approved this vault.
    ...
    function applyPnl(address[] calldata traders, int256[] calldata deltas) external onlySettlement {
        ...
        if (totalInternal > collateral.balanceOf(address(this))) revert BackingShortfall();
    }
    ...
    /// @notice Real collateral held by the vault. Proof-of-solvency: this should always
    ...
    function isSolvent() external view returns (bool) {
        return collateral.balanceOf(address(this)) >= totalInternal;
    }
}
