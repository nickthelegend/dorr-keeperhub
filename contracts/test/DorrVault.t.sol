...
/// Minimal 6-decimal ERC20 standing in for collateral in unit tests. The live
/// deployment uses the real FAssets collateral token on Coston2 — this exists only so
/// the invariants below can be exercised deterministically.
contract TestToken is ERC20 {
    constructor() ERC20("Test collateral", "tcollateral") {}
...
contract DorrVaultTest is Test {
    ...
    function setUp() public {
        collateral = new TestToken();
        vault = new DorrVault(address(collateral), owner);
        vm.prank(owner);
        vault.setSettlement(settlement);

        collateral.mint(alice, 1_000_000_000); // 1000 collateral (6dp)
    ...
    function test_WithdrawOwnFunds() public {
        ...
        assertEq(collateral.balanceOf(alice), 940_000_000);
    }
    ...
    function testFuzz_WithdrawNeverExceedsFree(uint96 dep, uint96 lock, uint96 wd) public {
        vm.assume(dep > 0);
        collateral.mint(alice, dep);
        vm.startPrank(alice);
        collateral.approve(address(vault), dep);
        vault.deposit(dep);
        vm.stopPrank();
