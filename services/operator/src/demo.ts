...
import { formatSize } from "./trading-math.js";
...
/**
 * One stage of the attack, in order.
 *
 * Deliberately *not* timestamped. An earlier version carried millisecond
 * offsets (0, 320, 640…) which read as measurements of something that happened
 * and were nothing of the sort — they were spacing for an animation. A number
 * that looks measured and isn't costs more credibility than the animation was
 * ever worth, so the stages are ordered and unnumbered.
 */
export interface AttackStep {
  /** Position in the sequence, 1-based. Order, not time. */
  step: number;
...
/**
 * Two orderings of the same trade: on a transparent venue the bot front-runs
 * and profits; on dorr it sees only the 32-byte commitment, fails to invert it,
 * and aborts.
 *
 * What is computed and what is asserted, precisely — because the whole project
 * turns on that distinction:
 *
 *   • the sandwich economics are solved against *this market's* live vAMM curve
 *     at the current Chainlink index. Real inputs, real constant-product math.
 *   • the brute force is real: it runs `bruteForceAttempts` actual SHA-256
 *     preimage guesses against the real commitment and reports the real count.
 *   • nothing here touches a chain. This is a model of what a sandwich against
 *     this curve would cost, not a measurement of one that happened.
 *
 * The last point is the one that matters. MEV Shield measures the same attack
 * for real, on Sepolia, with transaction hashes — so this is only ever the
 * explanation, and it says so in the UI rather than letting anyone assume.
 */
export function runAttackLab(p: AbParams): AttackLabResult {
  ...
  const sizeStr = formatSize(ab.victim.sizeBase);
  ...
  const publicRun = {
    steps: [
      { step: 1, actor: "bot", ok: true, text: "🤖 Sandwich bot scanning the public mempool / order flow…" },
      { step: 2, actor: "bot", ok: true, text: `👁️ Order spotted IN THE CLEAR: ${p.side} ${sizeStr} ${m.base} · ${p.leverage}x — full details visible` },
      { step: 3, actor: "bot", ok: true, text: `⚡ FRONT-RUN: bot buys ahead, pushing the mark to ${ab.public.botFrontRunPrice.toFixed(6)}` },
      { step: 4, actor: "victim", ok: true, text: `🎯 Victim's order executes at ${ab.public.victimEntry.toFixed(6)} — worse than fair (${idx.toFixed(6)})` },
      { step: 5, actor: "bot", ok: true, text: `💰 BACK-RUN: bot unwinds at ${ab.public.botExitPrice.toFixed(6)}, pocketing $${ab.public.botProfitUsd.toFixed(2)}` },
      { step: 6, actor: "victim", ok: false, text: `✗ SANDWICHED — victim overpaid $${ab.public.victimExtraCostUsd.toFixed(2)} (${ab.public.victimSlippageBps.toFixed(1)} bps)` },
    ] as AttackStep[],
  ...
  const privateRun = {
    steps: [
      { step: 1, actor: "bot", ok: true, text: "🤖 Sandwich bot scanning the public mempool / order flow…" },
      { step: 2, actor: "dorr", ok: false, text: `🔒 Only a commitment is public: ${commitment.slice(0, 18)}… — no side, size, price, or leverage` },
      { step: 3, actor: "bot", ok: false, text: `🔓 Bot attempts to crack the commitment — ${bruteForceAttempts.toLocaleString()} SHA-256 preimage guesses…` },
      { step: 4, actor: "dorr", ok: false, text: `❌ ${bruteForceMatches} / ${bruteForceAttempts.toLocaleString()} matches — the 128-bit nonce makes the search space 2¹²⁸ (infeasible)` },
      { step: 5, actor: "bot", ok: false, text: "❓ Bot has no direction, no size — a sandwich cannot even be constructed" },
      { step: 6, actor: "bot", ok: false, text: "🛑 ATTACK ABORTED — there is nothing to front-run" },
      { step: 7, actor: "victim", ok: true, text: `✓ Victim fills at the fair price ${ab.private.victimEntry.toFixed(6)} — bot profit $0.00` },
    ] as AttackStep[],
