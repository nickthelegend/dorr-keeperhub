...
const bad = (c: Context, msg: string, code: 400 | 404 | 500 = 400) => c.json({ error: msg }, code);
...
mev.get("/mev/status", async (c) => {
  ...
  try {
    ...
    const [reserveBase, reserveQuote, midPrice] = (await Promise.all([
 ...
 *
 * Only one duel runs at a time: two concurrent duels would sandwich each
 * other's trades and neither result would mean anything.
 */
let running = false;

mev.post("/mev/duel", async (c) => {
  if (!configured()) return bad(c, "MEV lab not configured — deploy it and set KEEPERHUB_API_KEY");
  if (running) return bad(c, "a duel is already running — they must not overlap", 400);
  ...
  if (!Number.isFinite(Number(amountIn)) || Number(amountIn) <= 0) {
    return bad(c, "amountIn must be a positive number");
  }
  if (!Number.isInteger(slippageBps) || slippageBps < 1 || slippageBps > 5000) {
    return bad(c, "slippageBps must be an integer between 1 and 5000");
  }
