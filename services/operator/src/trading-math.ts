 ...
 *
 * Fixed precision does not work across markets whose units differ by four
 * orders of magnitude. `toFixed(2)` renders a $200 BTC position as `0.00` — a
 * real position that reads as a broken engine — while eight decimals on a LINK
 * position is noise. So precision scales with magnitude, and a non-zero size
