/**
 * dorr trading core — ties vAMM execution, operator accounting, the order ZK
 * lifecycle and on-chain settlement together. Settlement runs as an async job
...
/** Trader secret bound into the commitment — derived per order (operator-held in v1). */
...
/**
 * Reserved open interest for a market (notional, mUSD): booked notional of every
 ...
 * private mode) publishes ONLY the hash. The seal + authority proof run
...
export function commitOrder(p: CommitParams): { order: DorrOrder; jobId: string } {
  ...
  if (p.marginUsd > free) throw new Error(`insufficient free margin: ${free.toFixed(2)} mUSD`);
  ...
  if (reservedOi + thisNotional > m.maxOiUsd) {
    throw new Error(
      `market ${p.marketId} open-interest cap reached (${reservedOi.toFixed(0)} + ${thisNotional.toFixed(0)} > ${m.maxOiUsd} mUSD) — reduce size or wait`,
    );
  }
...
export function executeOrder(orderId: string): { position: DorrPosition; jobId: string } {
  ...
  if (order.status === "cancelled" && order.cancelReason === "expired") {
    throw new Error(
      `this market order expired before it was executed — a market commit is priced at commit time and is only executable for ${Math.round(COMMIT_TTL_MS / 1000)}s. Its margin has been released; commit a new one`,
    );
  }
  if (order.status !== "committed") throw new Error(`order is ${order.status}`);
  // The keeper sweeps these, but it runs on an interval and this is the path
  // that would actually mis-fill. A market commit priced minutes ago must not
  // execute against the current index.
  if (order.orderType !== "limit" && commitAgeMs(order) >= COMMIT_TTL_MS) {
    throw new Error(
      `this market order was committed ${Math.round(commitAgeMs(order) / 1000)}s ago and has expired — its margin is released, commit a new one`,
    );
  }
  ...
  logEvent({
    ...
    address: order.address,
    marketId: order.marketId,
    detail: `${order.orderType === "limit" ? "Limit order filled" : "Opened"} ${order.side} ${formatSize(order.sizeBase)} @ ${fill.avgPrice.toFixed(6)} · ${order.leverage}x`,
  });

  // The fill is applied to the vAMM synchronously above, so the job exists to
  // give callers a uniform handle to poll rather than to wait on anything.
  const job = createJob("execute", position.id);
  const step = jobStep(job, "position opened");
  step.done({ detail: `${position.side} ${formatSize(position.sizeBase)} @ ${position.entryPrice.toFixed(6)}` });
  completeJob(job);
 ...
 * PnL to the account. The realized delta is left for the settlement keeper,
 * which batches it onto the chain through KeeperHub — this function moves no
 * money itself, and could not: the operator is not the vault's settlement
 * address.
 */
export function closePosition(
  ...
  acct.pnl += settledDeltaOf(pnl, fee, fundingPortion);
  ...
  if (f < 1 - 1e-9) {
    ...
    logEvent({
      ...
      detail: `Partial close ${(f * 100).toFixed(0)}% @ ${exitPrice.toFixed(6)} — realized ${(pnl - fee).toFixed(2)} mUSD`,
    });
    const pjob = createJob("close", pos.id);
    jobStep(pjob, `partial close ${(f * 100).toFixed(0)}% @ ${exitPrice.toFixed(6)}`).done({
      detail: `realized ${(pnl - fee).toFixed(2)} mUSD; ${formatSize(pos.sizeBase)} left open`,
    });
    completeJob(pjob);
  ...
  logEvent({
    ...
    detail:
      ...
      ` @ ${exitPrice.toFixed(6)} — PnL ${(pos.realizedPnl ?? 0).toFixed(2)} mUSD`,
  });

  // PnL is realised against the account synchronously above; pushing it to the
  // vault is the settlement keeper's job, on its own schedule.
  const job = createJob("close", pos.id);
  const step = jobStep(job, "position closed");
  step.done({
    detail: `${reason} @ ${exitPrice.toFixed(6)} · PnL ${(pos.realizedPnl ?? 0).toFixed(2)} mUSD`,
  });
  completeJob(job);
...
export function adjustMargin(positionId: string, deltaUsd: number): DorrPosition {
  ...
  if (deltaUsd > 0) {
    const free = acct.balance - acct.locked;
    if (deltaUsd > free) throw new Error(`insufficient free margin: ${free.toFixed(2)} mUSD`);
  ...
  logEvent({
    ...
    address: pos.address,
    marketId: pos.marketId,
    detail: `${deltaUsd > 0 ? "Added" : "Removed"} ${Math.abs(deltaUsd).toFixed(2)} mUSD margin — now ${pos.leverage.toFixed(1)}x`,
  });
  return pos;
}
  ...
  logEvent({
    ...
    detail:
      order.orderType === "limit"
        ? `Cancelled resting limit order — ${order.marginUsd.toFixed(2)} mUSD margin released`
        : `Cancelled committed order — ${order.marginUsd.toFixed(2)} mUSD margin released`,
  });
  return order;
}

/**
 * How long a committed market order stays executable.
 *
 * A market order commits against the index price at commit time, so executing
 * one much later fills at a price that has nothing to do with what was
 * committed. Worse, an order that is never executed holds its margin forever:
 * market commits are not resting orders, so nothing in the UI lists them, and
 * the locked balance has no visible cause. Commit and close the tab and that
 * margin is simply gone until someone reads the database.
 *
 * Two minutes is far longer than the commit→execute round trip actually takes
 * and short enough that nobody's collateral is stranded.
 */
export const COMMIT_TTL_MS = 120_000;

export function commitAgeMs(order: DorrOrder): number {
  return Date.now() - new Date(order.createdAt).getTime();
}

/**
 * Keeper: release margin held by market commits that were never executed.
 *
 * Limit orders are exempt — resting until their price is reached is the entire
 * point of one, and they are visible and cancellable in the UI.
 */
export function expireStaleCommits(): string[] {
  const st = getState();
  const expired: string[] = [];
  for (const o of st.orders) {
    if (o.status !== "committed" || o.orderType === "limit") continue;
    if (commitAgeMs(o) < COMMIT_TTL_MS) continue;
    const acct = account(o.address);
    acct.locked = Math.max(0, acct.locked - o.marginUsd);
    o.status = "cancelled";
    ...
    logEvent({
      ...
      address: o.address,
      marketId: o.marketId,
      detail: `Market commit expired unexecuted — ${o.marginUsd.toFixed(2)} mUSD margin released`,
    });
  }
...
export function addSealedOrder(p: {
  ...
  const acct = account(p.address);
  const free = acct.balance - acct.locked;
  if (p.maxMarginUsd > free) throw new Error(`insufficient free margin: ${free.toFixed(2)} mUSD`);
...
export async function settleSealedBatch(marketId: string): Promise<SealedBatchResult> {
  ...
  if (valid.length > 0) {
    ...
    // The membership root is recorded with the settlement below, and is
    // auditable off-chain only: publishing it would need a settlement contract
    // that re-checks the clearing price against Chainlink, which this
    // deployment does not have. The PnL it produces is settled on chain like
    // any other; the *root* is not.
  }
